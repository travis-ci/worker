...
var (
	gceFlags = []cli.Flag{
		...
		&cli.DurationFlag{
			...
			EnvVar: "GCE_BOOT_PRE_POLL_SLEEP,TRAVIS_WORKER_GCE_BOOT_PRE_POLL_SLEEP",
		},
		backendStringFlag("gce", "default-image-language", defaultGCELanguage,
			"Default language to use when looking up image",
			[]string{"DEFAULT_LANGUAGE"}),
		&cli.IntFlag{
			Name:   "disk-size",
			Value:  int(defaultGCEDiskSize),
			Usage:  "Disk size in GB",
			EnvVar: "GCE_DISK_SIZE,TRAVIS_WORKER_GCE_DISK_SIZE",
		},
		&cli.IntFlag{
			...
			EnvVar: "GCE_HARD_TIMEOUT_MINUTES,TRAVIS_WORKER_GCE_HARD_TIMEOUT_MINUTES",
		},
		&cli.StringSliceFlag{
			Name:   "image-aliases",
			Usage:  "Key=value pairs of image aliases",
			EnvVar: "GCE_IMAGE_ALIASES,TRAVIS_WORKER_GCE_IMAGE_ALIASES",
		},
		backendStringFlag("gce", "image-default", defaultGCEImage,
			"Default image name to use when none found",
		...
		&cli.StringSliceFlag{
			...
			EnvVar: "GCE_IMAGES,TRAVIS_WORKER_GCE_IMAGES",
		},
		...
		&cli.DurationFlag{
			...
			EnvVar: "GCE_STOP_PRE_POLL_SLEEP,TRAVIS_WORKER_GCE_STOP_PRE_POLL_SLEEP",
		},
		&cli.IntFlag{
			Name:   "upload-retries",
			Value:  int(defaultGCEUploadRetries),
			Usage:  "Number of times to attempt to upload script before erroring",
			EnvVar: "GCE_UPLOAD_RETRIES,TRAVIS_WORKER_GCE_UPLOAD_RETRIES",
		},
		&cli.DurationFlag{
			Name:   "upload-retry-sleep",
			Value:  defaultGCEUploadRetrySleep,
			Usage:  "Sleep interval between script upload attempts",
			EnvVar: "GCE_UPLOAD_RETRY_SLEEP,TRAVIS_WORKER_GCE_UPLOAD_RETRY_SLEEP",
		},
		backendStringFlag("gce", "zone", defaultGCEZone,
			"Zone name",
			[]string{"ZONE"}),
	}
